...
pub async fn watch_ingresses(
    ...
    info!("Started watching ingresses of class: [{ingress_class_name}]");

    while let Some(ev) = observer.try_next().await? {
        match ev {
...
pub async fn update_status(
    ...
    for ingress in ingresses.into_iter().filter(|ingress| {
        ...
        if ingress.status.is_none() {
            warn!("Ingress [{name}] in namespace [{namespace}] has no status");
            continue;
        }
            ...
            for path in &http.paths {
                if let Some(backend_service) = &path.backend.service {
                    ...
                    let port = u16::try_from(port_i32)
                        .map_err(|_| format!("Invalid port number: {port_i32}"))?;
...
fn handle_ingress_event(
    ...
    if !is_varnish_class(ingress, ingress_class_name) {
        info!("Skipping ingress [{ing_name}], it does not have the Varnish class.");
        return;
    }

    info!("Parsing ingress [{ing_name}]");
    match parse_ingress_spec(ingress) {
        Ok(bbs) => {
...
fn reconcile_backends(
    ...
    if let Err(e) = reload(&vcl) {
        error!("Failed to reload VCL: {e}");
    }
